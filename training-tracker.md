...
- **⚠️ Risk — the Android app has no offline/bundled copy of the site.** If `fittracker.se` ever goes away (domain expires — see the "Deployment + custom domain" entry above, expiry **2027-06-25**; DNS misconfigured; GitHub Pages disabled) or is unreachable, the native app shows a network error and is completely unusable — there is no fallback content baked into the APK. **The fix, if that ever happens:** switch `capacitor.config.json`'s `server` block to bundle the web assets locally instead of loading a remote URL — set `webDir` to a real copy of the site's files (`index.html`, `js/`, `styles.css`, etc.) and drop the `server.url` override entirely, then `npx cap sync android` and rebuild. This trades "always fresh" for "always works." See the matching note under "Native mobile app" in the TODO section below.
...
- **Domain dependency risk + fix:** see the "⚠️ Risk" bullet under "Native Android app (Capacitor)" above — the app loads `fittracker.se` live rather than bundling the site, so it stops working entirely if the domain/site goes down. Fix is to switch to a bundled `webDir` if/when that independence is worth losing "always fresh" for.
